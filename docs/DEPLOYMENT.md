...
5. 在 Nginx http 上下文加载生成的 `nginx`，在已有 preview HTTPS server 中 include 生成的 `locations`。先备份原配置、比较防止覆盖他人变更，再 `nginx -t`，通过才 reload。
...
- Nginx 新增 `/etc/nginx/conf.d/catsco-artifact-gateway-p0.conf`，只在 preview vhost 增加一个 include。原 vhost 保存在 `/etc/catsco-artifact-gateway/preview.before.conf`。
...
2. 从 preview vhost 移除本原型的 include；不要直接覆盖整个文件，除非确认相对备份无其他变更。
...
- 独立应用域名与证书，避免路径前缀兼容和共享 origin 问题。
